...
@pytest.fixture(params=["/types/related_articles/v1/articles"])
def get_url(request):
...
def recommend_response(monkeypatch):
    monkeypatch.setattr(
        related_articles, "recommend", lambda *args, **kwargs: GOOD_RESPONSE
    )
...
def test_default_params(client, get_url, params):
    with client as c:
        ...
        args = related_articles.get_v1_articles_params().parse_args()
