...
ArticleSpec = collections.namedtuple(
    "Article", ["pageviews", "title", "wikidata_id", "rank"]
)
...
legacy_doc = dict(
    ...
    params=dict(
        s="Source wiki project language code",
...
def get_v1_params():
    ...
    v1_params.add_argument(
        "seed", type=inputs.regex(r"^([^|]+(\|[^|]+)*)?$"), required=False
    )
    v1_params.add_argument(
        "include_pageviews", type=inputs.boolean, required=False, default=True
    )
    v1_params.add_argument(
        "search",
        type=str,
...
def recommend(
    ...
    if seed:
        ...
        candidates.extend(
            finder_map["mostpopular"](source, seed, max_candidates, campaign)
        )
