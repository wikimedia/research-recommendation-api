...
@pytest.fixture(params=["/types/translation/v1/articles", "/api/"])
def get_url(request):
    legacy_to_v1 = {
        "s": "source",
        "t": "target",
    ...
    return (
        lambda input_dict: request.param
        + "?"
        + urllib.parse.urlencode(
            {legacy_to_v1.get(k, k): v for k, v in input_dict.items()}
            if "v1" in request.param
            else input_dict
        )
    )
...
def test_generated_recommend_response_is_marshalled(client, get_url, monkeypatch):
    ...
    monkeypatch.setattr(translation, "finder_map", {"mostpopular": mock_finder})
    monkeypatch.setattr(
        filters, "apply_filters", lambda source, target, recs, campaign: recs
    )
