...
BAD_PAGEVIEW_RESPONSE = {
    "detail": (
        "The date(s) you used are valid, but we either do not have data for those date(s), or the project you "
...
@pytest.mark.parametrize(
    "body,status",
    [("", 404), ("bad json", 200), (requests.HTTPError("mocked exception"), 0)],
)
...
@pytest.mark.parametrize(
    "seed,query,expected_calls,seed_response,morelike_response",
    [
...
def test_wiki_search_paths(
    ...
    for query, count, morelike, response in search_pattern.values():
        ...
        responses.add(
            responses.GET, url, json=response, status=200, match_querystring=True
        )
    finder("en", seed, 10)
