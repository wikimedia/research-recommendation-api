...
class Type(flask_restplus.Resource):
    ...
    @api.doc(
        description="This returns the available subparts and the paths to their specs"
    )
    def get(self):
        types = []
        for blue in flask.current_app.iter_blueprints():
            if type(blue) is flask.Blueprint and blue not in (
                api.blueprint,
                gapfinder.gapfinder,
            ):
                types.append(
                    TypeSpec(
                        name=blue.name, spec_path=flask.url_for(blue.name + ".spec")
                    )._asdict()
                )
        return types
