...
def get(url, params=None, headers=None):
    log.debug("Get: %s", url)
    user_agent = (
        "WMF Research Gapfinder (https://recommend.wmflabs.org/; leila@wikimedia.org)"
    )
    if headers is None:
...
def get_relative_timestamp(relative_days):
    date_format = configuration.get_config_value(
        "single_article_pageviews", "date_format"
    )
    return (
        datetime.datetime.utcnow() + datetime.timedelta(days=relative_days)
...
def get_most_popular_articles(source, campaign=""):
    ...
    date = (datetime.datetime.utcnow() - datetime.timedelta(days=days)).strftime(
        date_format
    )
    ...
    if (
        "items" not in data
        or len(data["items"]) < 1
        or "articles" not in data["items"][0]
    ):
...
def get_related_articles(source, seed):
    ...
    try:
        response = get(
            endpoint, dict(source=source, seed=seed, count=500), headers=headers
        )
    except ValueError:
        return []
    return response
...
def get_pages_in_category_tree(source, category, count):
    ...
    while len(pages) < count:
        ...
        with multiprocessing.Pool(processes=len(current_categories)) as pool:
            results = pool.map(
                lambda category: get_category_members(source, category),
                current_categories,
            )
...
def set_headers_with_host_header(configuration, endpoint_name, source=""):
    ...
    if configuration.section_exists("endpoint_host_headers"):
        host_header = configuration.get_config_value(
            "endpoint_host_headers", endpoint_name
...
def get_formatted_endpoint(configuration, endpoint_name, source=""):
    ...
    Returns:
        str: The endpoint.
    """
    endpoint = configuration.get_config_value("endpoints", endpoint_name).format(
        source=source
    )
    return endpoint
